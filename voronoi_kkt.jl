...
function solve_voronoi_kkt(K, inst)
    ...
    # stationary
    @constraint(part_model, [j=1:J_size, k=1:K], -1*(1:J_size.== j).+sum(μ_1[j,k,j1,j2]*g_1_diff(j1,j2) for j1=1:J_size for j2=vcat([1:j1-1]...,[j1+1:J_size]...)) .+ μ_2[j,k]*g_2_diff.+sum(μ_3[j,k,l]*g_3_diff(l,k) for l=vcat([1:k-1]...,[k+1:K]...)) .== 0)
